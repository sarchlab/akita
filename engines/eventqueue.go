...
func NewEventQueue() *EventQueueImpl {
	...
	q.events = make([]core.Event, 0, 0)
	heap.Init(&q.events)
	return q
}

// Push adds an event to the event queue
func (q *EventQueueImpl) Push(evt core.Event) {
	// q.Lock()
...
func (q *EventQueueImpl) Pop() core.Event {
	// q.Lock()
	e := heap.Pop(&q.events).(core.Event)
	// q.Unlock()
	return e
}
...
// queue
func (q *EventQueueImpl) Peek() core.Event {
	// q.Lock()
	evt := q.events[0]
	// q.Unlock()
	return evt
...
func (q *InsertionQueue) Push(evt core.Event) {
	...
	for ele = q.l.Front(); ele != nil; ele = ele.Next() {
		if ele.Value.(core.Event).Time() > evt.Time() {
			break
		}
	}
	// q.lock.RUnlock()
...
func (q *InsertionQueue) Pop() core.Event {
	// q.lock.Lock()
	evt := q.l.Remove(q.l.Front())
	// q.lock.Unlock()
	return evt.(core.Event)
}
...
// the queue.
func (q *InsertionQueue) Peek() core.Event {
	// q.lock.RLock()
	evt := q.l.Front().Value.(core.Event)
	// q.lock.RUnlock()
	return evt
}
