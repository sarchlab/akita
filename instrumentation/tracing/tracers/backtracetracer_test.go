...
var _ = Describe("BackTraceTracer", func() {
	...
	It("should trace two tasks", func() {
		t.StartTask(tracing.Task{ID: "1"})
		t.StartTask(tracing.Task{ID: "2", ParentID: "1"})
	...
	It("should trace three tasks", func() {
		t.StartTask(tracing.Task{ID: "1"})
		t.StartTask(tracing.Task{ID: "2", ParentID: "1"})
		t.StartTask(tracing.Task{ID: "3", ParentID: "2"})
	...
	It("should end two tasks", func() {
		...
		t.EndTask(tracing.Task{ID: "3", ParentID: "2"})
		t.EndTask(tracing.Task{ID: "2", ParentID: "1"})
	...
	It("should print single tasks", func() {
		...
		mockTaskPrinter.EXPECT().Print(tracing.Task{ID: "1"})

		t.DumpBackTrace(tracing.Task{ID: "1"})
	})
	...
	It("should print three tasks", func() {
		...
		e3 := mockTaskPrinter.EXPECT().
			Print(tracing.Task{ID: "3", ParentID: "2"})
		e2 := mockTaskPrinter.EXPECT().
			Print(tracing.Task{ID: "2", ParentID: "1"}).
			After(e3)
		mockTaskPrinter.EXPECT().Print(tracing.Task{ID: "1"}).
			After(e2)

		t.DumpBackTrace(tracing.Task{ID: "3", ParentID: "2"})
	})
