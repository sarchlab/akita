...
func NewDirectConnection(engine core.Engine) *DirectConnection {
	c := DirectConnection{}
...
// DirectConnection.
func (c *DirectConnection) Attach(connectable core.Connectable) {
	c.EndPoints[connectable] = true
}

// Detach removes a Connectable from the end point list of the
// DirectConnection
func (c *DirectConnection) Detach(connectable core.Connectable) {
...
func (c *DirectConnection) Send(req core.Req) *core.Error {
	// if req.Src() == nil {
...
func (c *DirectConnection) Handle(evt core.Event) error {
	switch evt := evt.(type) {
	case *DeliverEvent:
