...
// Clone returns a copy of the transfer unit with a freshly generated ID. The
// message reference is copied verbatim; callers should ensure the message is
// immutable or safe for shared use.
func (t TransferUnit) Clone() TransferUnit {
	clone := t
	clone.Metadata = CloneMsgMetaData(t.Metadata)

	return clone
}

// CloneMsgMetaData returns a copy of the metadata with a freshly generated ID.
func CloneMsgMetaData(meta MsgMetaData) MsgMetaData {
	clone := meta
	clone.ID = nextID()
