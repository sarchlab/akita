...
// cycleCtx exposes immutable facts about the current slice of simulated time
// and allows staging follow-up events.
type cycleCtx interface {
	Now() sim.VTimeInSec
	StageEvent(sim.Event)
}
...
// commitAwareHandler mirrors the updated Handler interface that returns the
// next value for the associated state slot.
type commitAwareHandler interface {
	Handle(cycleCtx, sim.Event) (any, error)
}
...
func (e splitEvent) Handler() commitAwareHandler { return e.handler }
