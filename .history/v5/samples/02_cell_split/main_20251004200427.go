...
func (h *splitHandler) Handle(evt timing.Event) {
	...
	nextSplitTime := evt.Time() + jitter()
	h.scheduleNextSplitEvent(nextSplitTime, state.Cells)
	h.scheduleNextSplitEvent(nextSplitTime, split.id)
}

func (h *splitHandler) scheduleNextSplitEvent(now timing.VTimeInCycle, id int) {
	timeUntilNextSplit := timing.VTimeInCycle(h.rng.Intn(3) + 1)
	nextEvt := splitEvent{
		cycle:   now + timeUntilNextSplit,
		handler: h,
		id:      id,
	}

	if nextEvt.cycle < endCycle {
