...
2. Write for generative AI. We should provide sufficient documentation and examples so that vibe-coders can easily use Akita. 
3. Use a Go's coding style rather than some the Java style. Go prefers to define interfaces at use time. Therefore, we will remove some interface definition. For example, we will not provide a general `Engine` interface. Instead, we directly expose a `SerialEngine` struct and a `ParallelEngine` struct. Users can use them directly without going through an interface.
