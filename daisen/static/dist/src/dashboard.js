...
class Dashboard {
    ...
    setCanvas(canvas, pageBtnContainer, toolBar) {
        this._canvas = canvas;
    ...
    `;
        ...
        window.addEventListener('resize', () => {
            ...
            const paginationContainer = this._canvas.querySelector('.pagination-container');
    ...
    _addSecondarySelector(container) {
        const selectorGroup = document.createElement('div');
        selectorGroup.classList.add('input-group', 'ml-3');
        const button = document.createElement('div');
        button.classList.add('input-group-prepend');
    ...
    _filterInputChage() {
        ...
        this._filterTimer = setTimeout(() => {
            const filterInput = (document.getElementById("dashboard-filter-input"));
    ...
    _getParamsFromURL(simulation) {
        ...
        const dashboardFilter = (document.getElementById("dashboard-filter-input"));
        dashboardFilter.value = filter;
        let primaryAxisName = params.get("primary_y");
        if (primaryAxisName === null) {
            primaryAxisName = "ReqInCount";
        }
        const primaryAxisSelect = (document.getElementById("primary-axis-select"));
        ...
        const secondaryAxisSelect = (document.getElementById("secondary-axis-select"));
    ...
    _filter() {
        if (this._currFilter === "") {
            ...
            this._filteredNames = this._componentNames.filter((v) => v.search(re) >= 0);
        }
    ...
    _addPrevPageButton(ul, currPageNum) {
        const li = document.createElement("li");
    ...
    _addNextPageButton(ul, currPageNum, totalNumPages) {
        const li = document.createElement("li");
        li.classList.add("page-item");
    ...
    _switchToPage(pageNum) {
        ...
        this._addPaginationControl();
        this._renderPage();
        const pageInfo = this._pageBtnContainer.querySelector('.page-info');
    }
    _renderPage() {
