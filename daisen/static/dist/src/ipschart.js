...
class IPSChart {
    ...
    render() {
        ...
        this.calculateScale();
        this.renderBarGroup(this.data.StartCount, 'start-bar-group', 'green', 0);
        this.renderBarGroup(this.data.CompleteCount, 'complete-bar-group', 'red', 0.35);
        this.renderAxis();
    }
    calculateDimension() {
    ...
    renderBarGroup(data, groupName, color, xOffset) {
        ...
        startBarsEnter.merge(startBars)
            ...
            .attr('height', (d) => this.barRegionHeight - this.yScale(d));
    }
    renderAxis() {
        ...
        if (xAxisGroup.empty()) {
            xAxisGroup = this.svg.append('g')
                .attr('class', 'x-axis')
                .attr('transform', `translate(0, ${this.marginTop + this.barRegionHeight})`);
        }
