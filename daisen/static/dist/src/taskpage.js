...
export class TaskPage {
    constructor() {
        ...
        this._taskView = new TaskView(this._yIndexAssigner, new TaskRenderer(this, this._taskColorCoder), new XAxisDrawer());
        ...
        this._componentView = new ComponentView(this._yIndexAssigner, new TaskRenderer(this, this._taskColorCoder), new XAxisDrawer(), this._widget);
    ...
    setTimeRange(startTime, endTime, reload = false) {
    ...
    _layoutComponentView() {
        if (this._componentViewCanvas === null) {
            ...
            this._componentViewCanvas.appendChild(document.createElementNS("http://www.w3.org/2000/svg", "svg"));
    ...
    _layoutTaskView() {
        if (this._taskViewCanvas === null) {
            ...
            this._taskViewCanvas.appendChild(document.createElementNS("http://www.w3.org/2000/svg", "svg"));
    ...
    async showTask(task, keepView = false) {
        ...
        let parentTask = null;
        if (task.parent_id != "") {
            parentTask = await fetch(`/api/trace?id=${task.parent_id}`).then(rsp => rsp.json());
        }
    ...
    _updateTimeAxisAccordingToTask(task) {
    ...
    async _createComponentMilestoneView() {
        ...
        const response = await fetch(`/api/trace?where=${this._componentName}&starttime=${this._startTime}&endtime=${this._endTime}`);
        ...
        componentTasks.forEach(task => {
            if (task.steps && task.steps.length > 0) {
                task.steps.forEach(step => {
                    allMilestones.push(Object.assign(Object.assign({}, step), { taskId: task.id, taskKind: task.kind, taskWhat: task.what }));
                });
            }
        });
