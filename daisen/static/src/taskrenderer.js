...
class TaskRenderer {
  ...
  updateXAxis() {
    ...
    const tr = d3.transition("xAxisUpdate").duration(50).ease(d3.easeCubic);

    taskBarGroup
      .selectAll("rect")
      .attr("x", (d) => this._xScale(d.start_time))
      .attr(
        "width",
        (d) => this._xScale(d.end_time) - this._xScale(d.start_time)
      );

    // Update progress indicators
    svg
      .selectAll(".progress-indicator")
      .attr("cx", (d) => this._xScale(d.time));

    // Update progress lines
    // svg
    //   .selectAll(".progress-line")
    //   .attr("x1", (d) => this._xScale(d.start_time))
    //   .attr("x2", (d) => this._xScale(d.end_time));
  ...
  hightlight(task) {
    ...
    if (!task) {
      const tran = d3.transition("fade").duration(1000).ease(d3.easeCubic);
      taskBarGroup
        .selectAll("rect")
        // .transition(tran)
        .attr("opacity", 1.0)
    ...
    if (typeof task === "object") {
      const t = d3.transition("hightlight").duration(50).ease(d3.easeCubic);
      taskBarGroup
  ...
  render(tasks) {
    ...
    let taskBarGroup = svg.select(".task-bar");
    if (taskBarGroup.empty()) {
      taskBarGroup = svg.append("g").attr("class", "task-bar").lower();
    }

    const taskBars = taskBarGroup.selectAll("rect").data(tasks, (d) => d.id);
    ...
    const t = d3.transition("enter").duration(300).ease(d3.easeCubic);
