...
- **Current Akita V5 baseline:** the module remains `github.com/sarchlab/akita/v5` (`go.mod:1`) with Go language version `1.26.0` and toolchain `go1.26.2` (`go.mod:51-53`, `TOOLCHAIN_VERSIONS.md:8-16`). The current Akita checkout passes `go test ./...`; keep that command green as the local baseline before handing a downstream migration branch to mgpusim validation.
...
- **Generated-mock status:** generated mocks and generation hooks are now present in the checkout. The local Akita gate pins and installs `mockgen` before running `go generate -mod=readonly ./...` (`run_before_merge.sh:5`, `run_before_merge.sh:87`, `run_before_merge.sh:91`); package-level generation hooks also exist (`mem/vm/gmmu/generate_mocks.go:1-6`; examples: `mem/cache/writeback/mock_sim_test.go:1-20`, `mem/vm/gmmu/mock_port_test.go:1-20`, `noc/networking/switching/switches/mock_sim_test.go:1-20`). Migration work should preserve these generated fixtures or run generation before validation.
...
- **M25 validation-command scope:** `./run_before_merge.sh` is a local Akita Go build/lint/test gate, not a full merge-equivalent CI or downstream validation gate. It records Go version/env data, runs read-only dependency checks (`go list -mod=readonly -m all`, `go mod tidy -diff`), installs repository-pinned Go tools (`mockgen` v0.6.0, `golangci-lint` v2.9.0, `ginkgo` v2.25.1), then runs `go generate -mod=readonly ./...`, `go build -mod=readonly ./...`, `golangci-lint run --modules-download-mode=readonly ./...`, and `ginkgo -r --mod=readonly` (`run_before_merge.sh:5-7`, `run_before_merge.sh:80-95`). It also fails if `go.mod`, `go.sum`, or any tracked file changes at startup or after validation (`run_before_merge.sh:45-78`, `run_before_merge.sh:97-98`). It does not run Daisen frontend Node jobs, NOC/MEM Python acceptance tests, dependency-security validation, or downstream `mgpusim`/`mgpusim-dev` compile/smoke/benchmark validation; those remain separate follow-on gates (`.github/workflows/akita_test.yml:9-42`, `.github/workflows/akita_test.yml:70-112`).
...
2. **Akita baseline-health gate (currently green):** preserve the current `go test ./...` Akita baseline before and during downstream migration work. Generated mocks are covered by the local Akita Go gate through pinned `mockgen` installation and `go generate -mod=readonly ./...` (`run_before_merge.sh:87`, `run_before_merge.sh:91`), and package `//go:generate mockgen` directives exist (`mem/trace/generate.go:3`, `mem/vm/gmmu/generate_mocks.go:1-6`, `mem/vm/tlb/tlb_suite_test.go:12`), so downstream migration branches should keep generated test fixtures present or run generation before validation.
...
5. **Tooling/CI gate:** migration branch strategy, test matrix, and baseline validation criteria are defined in advance. Required commands should include at least `go list` package discovery, current Akita `go test ./...`, generated-mock preservation/regeneration checks, the local Akita Go build/lint/test gate, and explicitly separate CI/frontend/acceptance/downstream mgpusim smoke or acceptance tests. Do not count `./run_before_merge.sh` alone as full merge-equivalent coverage; it intentionally excludes Daisen frontend jobs and NOC/MEM Python acceptance jobs that are separate workflow jobs (`run_before_merge.sh:80-95`, `.github/workflows/akita_test.yml:9-42`, `.github/workflows/akita_test.yml:70-112`).
...
3. **Akita local Go build/lint/test gate:** `./run_before_merge.sh` is the repository-local Go gate for this Akita checkout. It uses repository-pinned tool versions (`mockgen` v0.6.0, `golangci-lint` v2.9.0, `ginkgo` v2.25.1), read-only dependency checks (`go list -mod=readonly -m all`, `go mod tidy -diff`), read-only generate/build/lint/test commands (`go generate -mod=readonly ./...`, `go build -mod=readonly ./...`, `golangci-lint run --modules-download-mode=readonly ./...`, `ginkgo -r --mod=readonly`), startup/post-validation tracked-file cleanliness checks, and a full local gate command span (`run_before_merge.sh:5-7`, `run_before_merge.sh:45-78`, `run_before_merge.sh:97-98`, `run_before_merge.sh:80-100`). Treat this as local Akita Go validation only unless a future branch adds frontend, acceptance, dependency-security, and downstream mgpusim coverage.
4. **CI/frontend/acceptance coverage (separate from the local gate):** full merge-equivalent Akita coverage would also run Daisen frontend install/test/build and NOC/MEM Python acceptance jobs (`.github/workflows/akita_test.yml:9-42`, `.github/workflows/akita_test.yml:70-112`). This branch's local gate does not include those commands or the separate dependency-security validation script.
5. **mgpusim compile gate:** all downstream packages compile after import/API conversion.
6. **mgpusim behavior gate:** representative GPU kernels/benchmarks and timing-memory acceptance tests pass against V4 reference outputs within agreed tolerances.
7. **monitoring/tracing gate:** a sample run produces usable task traces through `tracing.DBTracer` and live/replay endpoints through `monitoring`/`daisen` (`simulation/builder.go:102-127`, `monitoring/monitor.go:140-176`).
8. **checkpoint gate if used:** Save/Load tests prove quiescent-only checkpoint behavior and restored ID/time semantics (`simulation/saveload.go:54-60`, `simulation/saveload.go:192-210`).
