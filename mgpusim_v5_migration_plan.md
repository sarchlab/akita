...
- **Generated-mock status:** generated mocks and generation hooks are now present in the checkout. The local Akita gate installs pinned `mockgen` and runs `go generate -mod=readonly ./...` (`run_before_merge.sh:5`, `run_before_merge.sh:75`, `run_before_merge.sh:79`); package-level generation hooks also exist (`mem/vm/gmmu/generate_mocks.go:1-6`; examples: `mem/cache/writeback/mock_sim_test.go:1-20`, `mem/vm/gmmu/mock_port_test.go:1-20`, `noc/networking/switching/switches/mock_sim_test.go:1-20`). Migration work should preserve these generated fixtures or run generation before validation.
...
- **M25 validation-command scope:** `./run_before_merge.sh` is a local Akita Go build/lint/test gate, not a full merge-equivalent CI or downstream validation gate. It records Go version/env data, runs read-only dependency checks (`go list -mod=readonly -m all`, `go mod tidy -diff`), installs repository-pinned Go tools (`mockgen` v0.6.0, `golangci-lint` v2.9.0, `ginkgo` v2.25.1), then runs `go generate -mod=readonly ./...`, `go build -mod=readonly ./...`, `golangci-lint run --modules-download-mode=readonly ./...`, and `ginkgo -r --mod=readonly` (`run_before_merge.sh:5-7`, `run_before_merge.sh:68-83`). It also fails if `go.mod`, `go.sum`, or any tracked file changes during validation (`run_before_merge.sh:33-63`, `run_before_merge.sh:85-86`). It does not run Daisen frontend Node jobs, NOC/MEM Python acceptance tests, or downstream `mgpusim`/`mgpusim-dev` compile/smoke/benchmark validation; those remain separate follow-on gates (`.github/workflows/akita_test.yml:9-42`, `.github/workflows/akita_test.yml:70-112`).
...
2. **Akita baseline-health gate (currently green):** preserve the current `go test ./...` Akita baseline before and during downstream migration work. Generated mocks are covered by the local Akita Go gate through pinned `mockgen` installation and `go generate -mod=readonly ./...` (`run_before_merge.sh:75`, `run_before_merge.sh:79`), and package `//go:generate mockgen` directives exist (`mem/trace/generate.go:3`, `mem/vm/gmmu/generate_mocks.go:1-6`, `mem/vm/tlb/tlb_suite_test.go:12`), so downstream migration branches should keep generated test fixtures present or run generation before validation.
...
3. **Akita local Go build/lint/test gate:** `./run_before_merge.sh` is the repository-local Go gate for this Akita checkout. It uses repository-pinned tool versions (`mockgen` v0.6.0, `golangci-lint` v2.9.0, `ginkgo` v2.25.1), read-only dependency checks (`go list -mod=readonly -m all`, `go mod tidy -diff`), read-only generate/build/lint/test commands (`go generate -mod=readonly ./...`, `go build -mod=readonly ./...`, `golangci-lint run --modules-download-mode=readonly ./...`, `ginkgo -r --mod=readonly`), and tracked-file cleanliness checks (`run_before_merge.sh:5-7`, `run_before_merge.sh:33-63`, `run_before_merge.sh:68-88`). Treat this as local Akita Go validation only unless a future branch adds frontend, acceptance, and downstream mgpusim coverage.
