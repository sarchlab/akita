...
func (c *Connector) connectWithSwitch(xl, yl, z int) {
	// z1 := z - 1

	local := c.grid[xl][yl][z]
	// remote := c.grid[xr][yr][z]
	// localID := xl*6 + yl
	for i := 0; i < c.gridSize[0]; i++ {
		for j := 0; j < c.gridSize[1]; j++ {
			afterID := i*7 + j
			localID := xl*7 + yl
			if localID >= afterID {
				continue
			...
			remote := c.grid[i][j][z]
			localName := fmt.Sprintf("ToSW[%d][%d][%d]", i, j, z)
			remoteName := fmt.Sprintf("ToSW[%d][%d][%d]", xl, yl, z)
			portA, portB := c.createLink(local.sw, remote.sw, localName, remoteName)
			if local.rt.remote == nil {
				local.rt.remote = make(map[string]sim.Port)
			}
			if remote.rt.remote == nil {
				remote.rt.remote = make(map[string]sim.Port)
			}
			local.rt.remote[portA.Name()] = portA
			remote.rt.remote[portB.Name()] = portB
			// local.rt.remote = append(local.rt.remote, portA)
			// remote.rt.remote = append(remote.rt.remote, portB)
		}
