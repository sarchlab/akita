...
func newReq(ids timing.IDSource, src, dst messaging.RemotePort) readReq {
	return readReq{MsgMeta: messaging.MsgMeta{
		ID: ids.NewID(), Src: src, Dst: dst}}
}

func newRsp(ids timing.IDSource, src, dst messaging.RemotePort, rspTo uint64) readRsp {
...
func (m *clientMW) send() bool {
	...
	req := newReq(m.comp, port.AsRemote(), s.Dst)
...
func (m *cacheMW) forwardDown() bool {
	...
	downReq := newReq(m.comp, bottom.AsRemote(), m.comp.State.DownstreamDst)
...
func (m *cacheMW) respondUp() bool {
	...
	upRsp := newRsp(m.comp, top.AsRemote(), txn.upReq.Src, txn.upReq.ID)
	top.Send(upRsp)
...
func (m *memMW) Tick() bool {
	...
	port.Send(newRsp(m.comp, port.AsRemote(), req.Src, req.ID))
